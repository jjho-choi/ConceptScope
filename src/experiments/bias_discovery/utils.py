...
def plot_ref_images(
    checkpoint_name, latent_idx, concept_name=None, save_root="./out", show_title=False
):
...
def plot_concepts(
    ...
    all_slices = (
        concept_dict[str(selected_class)]["target"]
        + concept_dict[str(selected_class)]["context"][:len_context]
    )
    ...
    if save_fig:
        ...
        pio.write_image(
            fig, f"{save_dir}/classs_{selected_class}_concepts.svg", format="svg"
        )
    return data
...
def get_high_activating_images(
    ...
    sort_indices = np.argsort(latent_activation)[::-1][
        start_point : start_point + top_k
    ]
...
def plot_high_low_images(
    ...
    high_images_masks = get_sae_mask(
        high_images, sae, vit, cfg, latent_idx, resize_size=resize_size, device=device
    )
    low_images_masks = get_sae_mask(
        low_images, sae, vit, cfg, latent_idx, resize_size=resize_size, device=device
    )
    ...
    plot_images(
        ...
        resize_size=resize_size,
        show_plot=True,
        title=f"High Activating Images for Latent {latent_idx}",
    )
    plot_images(
        masked_high_images,
        resize_size=resize_size,
    ...
    plot_images(
        ...
        resize_size=resize_size,
        show_plot=True,
        title=f"Low Activating Images for Latent {latent_idx}",
    )
    plot_images(
        masked_low_images,
        resize_size=resize_size,
