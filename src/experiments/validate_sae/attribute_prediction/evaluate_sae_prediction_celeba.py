...
class SAEAttributePredictionEvaluator(ConceptScope):
    ...
    def compute_metric(self, target, latent_activations, high_indices):
        ...
        latent_indices = np.argsort(latent_activations[target][:, self.valid_latents].mean(axis=0))[-10:]
        high_indices = self.valid_latents[latent_indices]
        ...
        if dataset_name == "celeba":
            ...
            latent_indices = np.argsort(latent_activations[target][:, self.valid_latents].mean(axis=0))[-3:]
            latent_indices = self.valid_latents[latent_indices]
            # latent_indices = np.argsort(roc_scores)[-3:]
            latent_activation = latent_activations[:, latent_indices].mean(axis=1)
    ...
    def run(self, dataset_name, split):
        ...
        for cls_idx, attr in tqdm(enumerate(all_data["attr_list"]), desc="Evaluating Attributes"):
            ...
            # if dataset_name != "celeba":
            #     with open(f"{self.save_root}/{self.dir_name}/{dataset_name}/{checkpoint_name}/{concept_dict}.json", "r") as f:
