...
class SAESegmentationEvaluator(ConceptScope):
    ...
    def get_seg_mask_pred(self, image):
        inputs = ImageProcessor.process_model_inputs(
            {"image": [image]}, self.vit, self.device
        )
        ...
        masks = torch.Tensor(
            sae_act.reshape(1, sae_act.shape[1], feature_size, feature_size)
        )
        return masks
    ...
    def evaluate_test_dataset(self, dataset, class_latent_mapping):
        ...
        for i, sample in enumerate(tqdm(dataset)):
            ...
            mask = self.eval_helper.process_mask(
                sample["mask"], resize_size=self.resize_size
            )
            ...
            for j in range(len(class_latent_mapping)):
                ...
                pred_mask = self.eval_helper.resize_mask(
                    sae_mask[:, latent_idx : latent_idx + 1], self.resize_size
                )
    ...
    def get_latent_mapping(self, dataset, class_names):
        ...
        scores = np.zeros(
            (len(dataset), len(class_names), len(self.valid_latents)), dtype=np.float32
        )
        for i, sample in enumerate(tqdm(dataset)):
            image = self.eval_helper.process_image(sample["image"])
            mask = self.eval_helper.process_mask(
            ...
            sae_mask = self.get_seg_mask_pred(image)
            pred_masks = self.eval_helper.resize_mask(
                sae_mask[:, self.valid_latents], self.resize_size
            )
            for cls_idx, _ in enumerate((class_names)):
                ...
                for j, latent_idx in enumerate((self.valid_latents)):
                    ...
                    if len(np.where(pred_mask.flatten() > 0.5)[0]) == 0:
                        continue
                    scores[i, cls_idx, j] = self.eval_helper.evaluate_auprc(
    ...
    def run(self, num_train_samples=1000, num_test_samples=None):
        train_dataset = self.eval_helper.get_image_and_mask(
            "training", subsample=num_train_samples
        )
        test_dataset = self.eval_helper.get_image_and_mask(
            "validation", subsample=num_test_samples
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate SAE segmentation predictions"
    )
    parser.add_argument(
        "--save_root", type=str, default="./out", help="Root directory to save results"
    )
    parser.add_argument(
        "--dir_name",
        type=str,
        default="segmentation",
        help="Name of the directory to save results",
    )
    parser.add_argument(
        "--sae_path", type=str, required=True, help="Path to the SAE model checkpoint"
    )
    parser.add_argument(
